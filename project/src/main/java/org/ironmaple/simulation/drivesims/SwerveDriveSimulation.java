...
public class SwerveDriveSimulation extends AbstractDriveTrainSimulation {
    ...
    public SwerveDriveSimulation(DriveTrainSimulationConfig config, Pose2d initialPoseOnField) {
        ...
        super.setLinearDamping(1.4f);
        super.setAngularDamping(1.4f);
    ...
    private void simulateChassisFrictionForce() {
        ...
        double speedsDiffMagnitude =
                Math.min(
                        FRICTION_FORCE_GAIN * totalGrippingForce * floorAndModuleSpeedsDiffFieldRelative.getNorm(),
                        totalGrippingForce);
        double speedsDiffDirection = MapleCommonMath.getAngle(floorAndModuleSpeedsDiffFieldRelative).getRadians();
        final Vec2 speedsDifferenceFrictionForce =
                createVec2FromPolar((float) speedsDiffMagnitude, (float) speedsDiffDirection);
        ...
        double centripetalMagnitude =
                previousModuleSpeedsFieldRelative.getNorm() * orbitalAngularVelocity * config.robotMass.in(Kilograms);
        final Vec2 centripetalFrictionForce =
                createVec2FromPolar((float) centripetalMagnitude, (float) centripetalForceDirection.getRadians());
        previousModuleSpeedsFieldRelative =
                GeometryConvertor.getChassisSpeedsTranslationalComponent(moduleSpeedsFieldRelative);

        /* apply force to physics engine */
        final Vec2 totalFrictionForceUnlimited = centripetalFrictionForce.add(speedsDifferenceFrictionForce);
        double totalMagnitude = Math.min(totalGrippingForce, totalFrictionForceUnlimited.length());
        double totalDirection = Math.atan2(totalFrictionForceUnlimited.y, totalFrictionForceUnlimited.x);
        final Vec2 totalFrictionForce = createVec2FromPolar((float) totalMagnitude, (float) totalDirection);
        super.applyForce(totalFrictionForce);
    }

    /** Helper method to create Vec2 from polar coordinates */
    private static Vec2 createVec2FromPolar(float magnitude, float angle) {
        return new Vec2(magnitude * (float) Math.cos(angle), magnitude * (float) Math.sin(angle));
    }
    ...
    private void simulateChassisFrictionTorque() {
        final double desiredRotationalMotionPercent =
                        Math.abs(getDesiredSpeed().omegaRadiansPerSecond / maxAngularVelocity().in(RadiansPerSecond)),
                actualRotationalMotionPercent = Math.abs(getAngularVelocity() / maxAngularVelocity().in(RadiansPerSecond)),
                differenceBetweenFloorSpeedAndModuleSpeed =
                        getModuleSpeeds().omegaRadiansPerSecond - getAngularVelocity(),
                grippingTorqueMagnitude = moduleSimulations[0].config.getGrippingForceNewtons(gravityForceOnEachModule)
        ...
        if (actualRotationalMotionPercent < 0.01 && desiredRotationalMotionPercent < 0.02) super.setAngularVelocity(0);
            ...
            super.applyTorque((float) Math.copySign(
                    Math.min(
    ...
    private void simulateModulePropellingForces() {
        for (int i = 0; i < moduleSimulations.length; i++) {
            final Vec2 moduleWorldPosition = getWorldPoint(GeometryConvertor.toBox2dVec2(moduleTranslations[i]));
